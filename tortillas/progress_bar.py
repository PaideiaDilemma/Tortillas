...
class ProgressBar:
    ...
    def create_counters(self, total: int):
        ...
        bar_format = (
            "{desc}{desc_pad} {count_00:d}/{total:d}|{bar}| "
            + "R:"
            + term.blue("{count_0:{len_total}d}")
            + " "
            + "S:"
            + term.green("{count_1:{len_total}d}")
            + " "
            + "F:"
            + term.red("{count_2:{len_total}d}")
            + " "
        )

        counter = self.progress_manager.counter(
            total=total,
    ...
    def update_counter(
        ...
        if from_counter and from_counter in self.Counter:
            self.counters[counter].update_from(self.counters[from_counter], incr=incr)
            return
