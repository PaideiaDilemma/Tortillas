...
class QemuInterface:
    ...
    def __exit__(
        ...
        if exc_type and exc_value:
            self.logger.error(
                "Error while running tests", exc_info=(exc_type, exc_value, traceback)
            )

    def _popen_qemu(self) -> subprocess.Popen:
        if self.arch in ("x86_64", "x86/64"):
            cmd = (
                f"qemu-system-x86_64 -m 8M -cpu qemu64 "
            ...
            cmd = (
                f"qemu-system-i386 -m 8M -cpu qemu32 "
                f"-drive file={self.qcow2_path},index=0,media=disk "
                f"-debugcon file:{self.log_file} "
                f"-monitor pipe:{self.fifos} "
                "-nographic -display none -serial /dev/null"
...
class InterruptWatchdog:
    ...
    def wait_until(
        self, int_num: int, int_regs: dict[str, int], timeout: int
    ) -> Status:
        """
    ...
    @staticmethod
    def match_registers(search_regs: dict[str, int], int_regs: dict[str, int]) -> bool:
        """
    ...
    @staticmethod
    def search_interrupt(
        int_num: int, search_regs: dict[str, int], lines: list[str]
    ) -> dict[str, int] | None:
        ...
        for index, line in enumerate(lines):
            ...
            for block_size in [21] + list(range(1, 30)):
                if (
                    index + block_size + 1 >= len(lines)
                    or "v=" in lines[index + block_size]
                ):
                    break
            ...
            interrupt = "".join(
                (
                    line.strip("\n") + " "
                    for line in lines[index + 1 : index + block_size]
                )
            )
